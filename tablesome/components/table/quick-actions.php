    ...
    class Quick_Actions
    {
        public function modify_table_row_actions($actions, $post)
        {
            ...
            if (tablesome_fs()->can_use_premium_code__premium_only()) {
                ...
                /***
                 * Checking the post-ID is exists in the global tablesome tables count collection.
                 * If it's exists, get the table records count by post-ID
                 */
                // if (isset($tablesome_tables_count_collection[$post->ID])) {
                //     $records_count = $tablesome_tables_count_collection[$post->ID];
                // }

                /**
                 * Incase, the post-ID doesn't exists the collection use the below workflow.
                 */
                // if (!isset($tablesome_tables_count_collection[$post->ID])) {
                //     $crud = new \Tablesome\Includes\Db\CRUD();
                //     $records_count = $crud->get_records_count($post->ID);
                // }

                if ($records_count > TABLESOME_COPY_RECORDS_LIMIT) {
                    return $actions;
                }
            }
