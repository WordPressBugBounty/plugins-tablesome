    ...
    class Mailchimp_Add_Contact extends Action
    {
        ...
        public function can_add_contact($action_meta, $data)
        {
            ...
            if (tablesome_fs()->can_use_premium_code__premium_only()) {
                ...
                $conditions = isset($action_meta['conditions']) ? $action_meta['conditions'] : [];
                $conditions_validated = $this->is_conditions_are_valid($conditions, $data);
        ...
        public function is_conditions_are_valid($conditions, $data)
        {
            ...
            foreach ($conditions as $condition) {
                ...
                if ($this->trigger_class->get_config()['integration'] == 'wpforms' && !empty($value) && intval($value)) {
                    $trigger_value = $this->trigger_class->get_field_option_id_by_value(array('field' => $field, 'value' => $value, 'trigger_value' => $trigger_value));
                }
        ...
        public function add_contact_to_the_lists($action_meta, $subscriber_data)
        {
            ...
            if (tablesome_fs()->can_use_premium_code__premium_only()) {
                $formatted_subscriber_data['tags'] = $this->get_tag_names_by_ids($action_meta);
            }
