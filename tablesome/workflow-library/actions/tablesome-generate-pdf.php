...
if (!class_exists('\Tablesome\Workflow_Library\Actions\Tablesome_Generate_Pdf')) {
    class Tablesome_Generate_Pdf extends Action
    {
        ...
        public function do_action($trigger_class, $trigger_instance)
        {
            ...
            // error_log('generate_pdf do_action() $this->fields : ' . print_r($this->fields, true));
            // error_log('generate_pdf do_action() $fields_to_use : ' . print_r($fields_to_use, true));

            if (tablesome_fs()->can_use_premium_code__premium_only()) {
                $file_info = $this->create_pdf($fields_to_use);
                $this->add_information_to_workflow_data($file_info);
            }
        ...
        protected function get_signature_placeholders($source_data)
        {
            ...
            foreach ($source_data as $field_name => $field_data) {
                ...
                $is_signature_type = isset($field_data['type']) && in_array(strtolower($field_data['type']), ['signature', 'e_signature', 'esignature', 'pafe_signature']);
                ...
                if ($is_signature || $is_signature_type) {
                    ...
                    if (!empty($file_url)) {
                        ...
                        $signature_width = isset($this->action_meta['signature_width'])
                            ? absint($this->action_meta['signature_width'])
                            : 300;
                        ...
                        $img_tag = sprintf(
                            '<br><img src="%s" width="%d" datasig="1"><br>',
                            esc_url($file_url),
                            $signature_width
                        );
        ...
        public function create_pdf($fields)
        {
            ...
            $pdf = new Tablesome_PDF_HTML('P', 'mm', 'A4', $fields);

            // Set configurable image width (default 300px)
            $image_width = isset($this->action_meta['image_width'])
                ? absint($this->action_meta['image_width'])
                : 300;
